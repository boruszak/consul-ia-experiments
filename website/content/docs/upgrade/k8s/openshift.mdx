...
1. Update Consul's configuration in your `values.yaml` file. To upgrade the image version from v1.18.0 to v1.18.2, edit the `global.image` stanza to `image: registry.connect.redhat.com/hashicorp/consul:1.18.2-ubi`

1. Run `helm diff upgrade` with your updated file.
...
1. If `consul-server, StatefulSet` appears, it means your Consul server StatefulSet will be redeployed when you apply the updated Helm values.
  ...
  If your StatefulSet needs to be redeployed, follow the regular upgrade pattern by redeploying servers one by one. Refer to [Upgrading Consul](/consul/docs/upgrade) for more information.



## Upgrade Helm chart version

Before apply your updated `values.yaml` file, you may need to update the version of your Consul Helm chart.
...
1. Specify the chart version you in a `helm upgrade` command. The following example updates the Helm chart to version `1.4.9`.
