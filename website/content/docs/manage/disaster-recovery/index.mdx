...
This topic provides an overview of the architectural considerations and recommendations for preparing a disaster recovery strategy for your Consul cluster.
...
**Recovery point objective (RPO)** - The maximum amount of data loss that can be incurred from a disaster, failure, or comparable event. This is measured as a unit of time and there is usually a 1-to-1 correlation between RPO and backup frequency. 
...
**Disaster recovery plan (DRP)** - A formal document created by an organization that contains the processes used to recover access to systems and data after a catastrophic event. DRPs will typically also include a set of processes for testing and validating disaster recovery procedures and establish a regular cadence for these events.  
...
You should regularly test and validate the restore process for critical systems to ensure that everything works as expected. This is typically defined within a [Disaster Recovery Plan](#glossary) (DRP). 
...
## Single-cluster disaster recovery
