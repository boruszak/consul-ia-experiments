...
Once the setup is complete, you can start the CRC service that runs OpenShift Local with the following command. The command will perform a few system checks to ensure your system meets the minimum requirements and will then ask you to provide an image pull secret. You should have your [Red Hat account](https://console.redhat.com/openshift/create/local) open so that you can easily copy your image pull secret when prompted.
...
Make note of this command, as you will use it to login to OpenShift Local (CRC) on your development
host later.
...
You must create an image pull secret before authenticating to the RedHat Registry and pulling images from the container registry. You must first create a registry service account on the [RedHat Customer Portal](https://access.redhat.com/terms-based-registry/), and then download and apply the OpenShift secret that is associated with the registry service account as shown below. In this case, the name of the downloaded file was `15490118-openshift-secret.yml` - yours will differ.
...
You will get a confirmation for the creation of a Kubernetes secret, alongside its name. Make a note of this name now, as later you will have to use it in the Helm chart values file, specifically in the `imagePullSecrets` stanza. In this case, the name is `15490118-openshift-secret-secret`.
...
To customize your deployment, you can pass a YAML configuration file to be used during the deployment. Any values specified in the values file will override the Helm chart's default settings. The following example file sets the `global.openshift.enabled` entry to true, which is required to operate Consul on OpenShift. Generate a file named `values.yaml` that you will reference in the `helm install` command later. Don't forget to modify the `imagePullSecrets` variable to the name generated in the previous step.
...
global:
  name: consul
  datacenter: dc1
  image: registry.connect.redhat.com/hashicorp/consul:1.20.3-ubi
...
hashicorp/consul	1.6.1        	1.20.2     	Official HashiCorp Consul Chart
```
...
```shell-session
$ oc import-image hashicorp/consul:1.20.3-ubi --from=registry.connect.redhat.com/hashicorp/consul:1.20.3-ubi --confirm
...
- Use `consul-helm` chart version `1.6.1`


```shell-session
$ helm install consul hashicorp/consul --values values.yaml --create-namespace --namespace consul --version "1.6.1" --wait
```
...
In order to install the latest version of the Consul K8s CLI depending on your operating system, follow the instructions [here](/consul/docs/k8s/installation/install-cli#install-the-cli). You can review the official [Consul K8S CLI documentation](/consul/docs/reference-cli/consul-k8s) to learn more about additional settings.
...
consul-server-0     10.217.0.106:8301  alive   server  1.20.3  2         dc1  default    <all>
crc-dzk9v-master-0  10.217.0.104:8301  alive   client  1.20.3  2         dc1  default    <default>
```
...
The Consul server you created is not ready for production. The chart was installed with an insecure configuration of Consul that you still need to secure. Refer to the [Secure Consul](/consul/docs/secure/) to learn how you can secure Consul servers for use in production environments.

For more information on the Consul Helm chart configuration options, review the [consul-helm chart documentation](/consul/docs/reference/k8s/helm).